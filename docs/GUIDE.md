...
Clients verify the server token's self-signature and detached DTLS fingerprint assertion. To match Bedrock, server identity `exp`/`nbf` claims are not enforced; `iat` must remain within a bounded 60-second clock-skew window. Servers apply normal temporal validation to client identity tokens and verify proof of fingerprint possession. `allow_anonymous` defaults to false. Possession of a cpk alone is not issuer authentication: configure `verify_client` to validate your trusted issuer/account policy. The callback must return a trusted public key; null rejects the identity. An alternative returned key has its fingerprint proof checked again.
...
Default native callback queue: 1 MiB and 512 entries, shared by signaling and channel fragments. It holds one maximum-sized accepted signaling event or four maximum-sized fragments; queue exhaustion fails the peer. The default remains fail-closed for every event type. `native.drop_unreliable_on_pressure` optionally drops only well-formed, single-fragment unreliable messages while reserving configurable bytes and entries for signaling and reliable traffic. Malformed unreliable frames, signaling, and reliable fragments remain fail-closed. `Connection.callbackStats()` reports dropped unreliable packets plus queue byte and entry high-water marks. Default message cap: 16 MiB. Reassembly grows geometrically on demand, capped at the configured maximum, and reuses capacity. Outgoing native buffered data is capped before submission, including fragment headers. Server counts, HTTP negotiation workers, and pending accepts are bounded.

A default connection initially owns about 2.5 MiB of queue, packet, negotiation, and send buffers plus small structs and native WebRTC state. Once established and queued signaling has drained, the 1 MiB negotiation buffer is released, leaving about 1.5 MiB. Packet and send buffers are sized from the configured message limit, capped at one 256 KiB fragment. Reassembly capacity is additional and retained for reuse until destroy. Tune `native.queue_bytes`/`queue_entries` for workload needs (queue bytes must fit at least one maximum segment). Incoming SDP and trickled ICE candidates share the bounded `maximum_remote_candidates` budget (32 by default) and are rejected before reaching libdatachannel when exhausted. `close()` remains immediate; `closeGracefully()` rejects later sends, waits on libdatachannel buffered-amount-low callbacks, and force-closes after `graceful_shutdown_timeout_ms` (2 seconds by default). Cancellation or native failure also force-closes. No allocation occurs in the Zig native message callback, discovery codec, pure framing codec, or byte-ring operations.
...
LAN discovery encryption and its fixed compatibility key provide wire-format integrity, not peer authentication: the key is public to compatible implementations. Treat discovery metadata and endpoints as untrusted. Connection identity is established only by the signed SDP fingerprint assertion; keep allow_anonymous disabled and configure verify_client for issuer/account policy.