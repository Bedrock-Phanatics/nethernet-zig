...
pub const Urls = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, credentials: Credentials) !Urls {
        ...
        for (credentials.ice_servers) |server| {
            if (server.urls.len > maximum_servers - count) return error.TooManyIceServers;
        ...
        var filled: usize = 0;
        errdefer for (values[0..filled]) |value| allocator.free(std.mem.span(value));

        for (credentials.ice_servers) |server| {
            for (server.urls) |url| {
                const too_long = url.len > maximum_url_length or
                    server.username.len > maximum_credential_length or
                    server.password.len > maximum_credential_length;
                if (too_long or std.mem.indexOfScalar(u8, url, 0) != null) return error.InvalidIceServer;
                ...
                const is_turn = std.ascii.eqlIgnoreCase(scheme, "turn") or std.ascii.eqlIgnoreCase(scheme, "turns");
                if (!is_stun and !is_turn) return error.InvalidIceServer;

                const has_credentials = server.username.len != 0 or server.password.len != 0;
                const owned = if (is_stun or !has_credentials)
                    try allocator.dupeZ(u8, url)
                else
                    try authenticatedUrl(allocator, scheme, url[colon + 1 ..], server.username, server.password);
...
test "TURN credentials escape delimiters and flatten URLs" {
    const urls = try Urls.init(std.testing.allocator, .{
        .ice_servers = &.{.{
            .username = "user:@",
            .password = "p@ss",
            .urls = &.{ "turn:relay.example:3478?transport=udp", "stun:stun.example:3478" },
        }},
    });
    defer urls.deinit();
