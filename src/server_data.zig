...
const Reader = struct {
    ...
    fn take(self: *Reader, len: usize) Error![]const u8 {
        if (len > self.data.len - self.offset) return error.MalformedPacket;
    ...
    fn uint(self: *Reader) Error!u32 {
        ...
        for (0..5) |i| {
            ...
            if (i == 4 and b > 15) return error.MalformedPacket;
...
const Writer = struct {
    ...
    fn put(self: *Writer, data: []const u8) Error!void {
        if (data.len > self.data.len - self.offset) return error.NoSpaceLeft;
...
pub const ServerData = struct {
    ...
    pub fn encode(self: ServerData, output: []u8) Error![]const u8 {
        var w = Writer{ .data = output };
        try w.byte(6);
        try w.string(self.server_name);
        try w.string(self.level_name);
        try w.int(self.game_type);
        try w.fixed(self.player_count);
        try w.fixed(self.max_player_count);
        inline for (.{ self.editor_world, self.hardcore, self.accepts_online_auth, self.accepts_self_signed_auth }) |value| try w.byte(@intFromBool(value));
        try w.string(self.nonce);
        try w.int(self.transport_layer);
        try w.int(self.connection_type);
        return output[0..w.offset];
    }

    pub fn decode(input: []const u8) Error!ServerData {
        var r = Reader{ .data = input };
        if (try r.byte() != 6) return error.MalformedPacket;
        const result: ServerData = .{
            .server_name = try r.string(),
            .level_name = try r.string(),
            .game_type = try r.int(),
            .player_count = try r.fixed(),
            .max_player_count = try r.fixed(),
            .editor_world = try r.byte() != 0,
            .hardcore = try r.byte() != 0,
            .accepts_online_auth = try r.byte() != 0,
            .accepts_self_signed_auth = try r.byte() != 0,
            .nonce = try r.string(),
            .transport_layer = try r.int(),
            .connection_type = try r.int(),
        };
        if (r.offset != input.len) return error.MalformedPacket;
        return result;
    }
    ...
    pub fn fromPong(pong: []const u8) Error!ServerData {
        ...
        for (&parts) |*part| part.* = fields.next() orelse return error.MalformedPacket;
        const game = std.mem.trim(u8, parts[8], " \r\n\t");
        return .{
            .server_name = parts[1],
            .level_name = parts[7],
            .player_count = std.fmt.parseInt(i32, std.mem.trim(u8, parts[4], " \t"), 10) catch 0,
            .max_player_count = std.fmt.parseInt(i32, std.mem.trim(u8, parts[5], " \t"), 10) catch 0,
            .game_type = if (std.ascii.eqlIgnoreCase(game, "CREATIVE")) 1 else if (std.ascii.eqlIgnoreCase(game, "ADVENTURE")) 2 else 0,
...
test "version six advertisement matches wire fixture" {
    const expected = [_]u8{ 6, 6, 's', 'e', 'r', 'v', 'e', 'r', 5, 'w', 'o', 'r', 'l', 'd', 4, 1, 0, 0, 0, 8, 0, 0, 0, 0, 1, 1, 1, 5, 'n', 'o', 'n', 'c', 'e', 4, 8 };
    const value = ServerData{ .server_name = "server", .level_name = "world", .game_type = 2, .player_count = 1, .max_player_count = 8, .hardcore = true, .accepts_online_auth = true, .accepts_self_signed_auth = true, .nonce = "nonce" };
    var buffer: [128]u8 = undefined;
    try std.testing.expectEqualSlices(u8, &expected, try value.encode(&buffer));
    const decoded = try ServerData.decode(&expected);
    try std.testing.expectEqualStrings(value.server_name, decoded.server_name);
    for (0..expected.len) |length| try std.testing.expectError(error.MalformedPacket, ServerData.decode(expected[0..length]));
    try std.testing.expectError(error.MalformedPacket, ServerData.decode(&.{ 6, 255, 255, 255, 255, 127 }));
}

test "signed varint boundaries and pong conversion" {
    var buffer: [128]u8 = undefined;
    for ([_]i32{ std.math.minInt(i32), -1, 0, 1, std.math.maxInt(i32) }) |value| {
        const data = ServerData{ .game_type = value, .connection_type = value };
        const decoded = try ServerData.decode(try data.encode(&buffer));
        try std.testing.expectEqual(value, decoded.game_type);
        try std.testing.expectEqual(value, decoded.connection_type);
    }
    const pong = try ServerData.fromPong("MCPE;server;1;2;3;4;5;world; Creative ");
