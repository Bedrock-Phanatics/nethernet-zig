...
pub fn main(init: std.process.Init) !void {
    ...
    const sample_capacity = @min(
        @as(usize, 1_000_000),
        @max(@as(usize, 4096), config.connections * 4096),
    );
    var metrics: Metrics = .{};
    ...
    while (!stop) {
        for (pairs, 0..) |*pair, pair_index| {
            ...
            if ((config.duration_ms != 0 and elapsed_ns >= deadline_ns) or
            ...
            for (0..config.burst) |_| {
                ...
                if (metrics.latencies_ns.items.len < sample_capacity) {
                    metrics.latencies_ns.appendAssumeCapacity(@intCast(
                        sent.durationTo(std.Io.Clock.awake.now(io)).nanoseconds,
                    ));
                }
            }
    ...
    for (pairs) |pair| recordFinalStats(&metrics, pair);
...
fn recordFinalStats(metrics: *Metrics, pair: Pair) void {
    const client = pair.client.callbackStats();
    const server = pair.server.callbackStats();
    metrics.dropped_unreliable +=
        client.dropped_unreliable_packets + server.dropped_unreliable_packets;
    updateHighWater(metrics, pair);
}
...
fn printReport(
    ...
    const format =
        ...
        "\"connections\":{d},\"duration_ms\":{d},\"message_limit\":{d}," ++
        "\"payload_size\":{d},\"rate\":{d},\"burst\":{d},\"profile\":\"{s}\"," ++
    ...
    std.debug.print(format, .{
        ...
        @as(f64, @floatFromInt(setup_ns)) / std.time.ns_per_ms,
        metrics.messages,
        metrics.bytes,
        @as(f64, @floatFromInt(metrics.messages)) / seconds,
        @as(f64, @floatFromInt(metrics.bytes)) / seconds / (1024 * 1024),
        percentile(metrics.latencies_ns.items, 50),
...
fn parseArgs(args: std.process.Args, allocator: std.mem.Allocator) !Config {
    ...
    while (iterator.next()) |argument| {
        if (std.mem.eql(u8, argument, "--connections")) {
            result.connections = try parse(usize, iterator.next());
        } else if (std.mem.eql(u8, argument, "--duration-ms")) {
            result.duration_ms = try parse(u64, iterator.next());
            duration_set = true;
        } else if (std.mem.eql(u8, argument, "--messages")) {
            result.message_limit = try parse(u64, iterator.next());
            messages_set = true;
        } else if (std.mem.eql(u8, argument, "--payload-size")) {
            result.payload_size = try parse(usize, iterator.next());
        } else if (std.mem.eql(u8, argument, "--rate")) {
            result.rate = try parse(u64, iterator.next());
        } else if (std.mem.eql(u8, argument, "--burst")) {
            result.burst = try parse(usize, iterator.next());
        } else if (std.mem.eql(u8, argument, "--churn-messages")) {
            result.churn_messages = try parse(u64, iterator.next());
        } else if (std.mem.eql(u8, argument, "--timeout-ms")) {
            result.timeout_ms = try parse(u32, iterator.next());
        } else if (std.mem.eql(u8, argument, "--reliability")) {
            const value = iterator.next() orelse return error.MissingArgument;
            result.reliability = std.meta.stringToEnum(
                @TypeOf(result.reliability),
                value,
            ) orelse return error.InvalidArgument;
        } else if (std.mem.eql(u8, argument, "--profile")) {
            const value = iterator.next() orelse return error.MissingArgument;
            result.profile = std.meta.stringToEnum(
...
fn receiveDeadline(
    ...
    while (true) {
        ...
        if (started.durationTo(std.Io.Clock.awake.now(connection.io)).toMilliseconds() >=
            timeout_ms)
        {
            return error.Timeout;
        }
...
fn writePayload(data: []u8, sequence: u64) void {
    ...
    for (data[16..], 16..) |*byte, index| {
        byte.* = @truncate(sequence +% index);
    }
}

fn validPayload(data: []const u8, sequence: u64, size: usize) bool {
    if (data.len != size) return false;
    if (std.mem.readInt(u64, data[0..8], .little) != sequence) return false;
    if (std.mem.readInt(u64, data[8..16], .little) != ~sequence) return false;

    for (data[16..], 16..) |byte, index| {
        if (byte != @as(u8, @truncate(sequence +% index))) return false;
    }
